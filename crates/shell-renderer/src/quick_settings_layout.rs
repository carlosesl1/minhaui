...
use crate::{
    DipPoint, DipRect, QuickSettingsAudioOutputId, QuickSettingsAudioSessionId,
    QuickSettingsChoiceId, QuickSettingsHit, QuickSettingsMediaAction, QuickSettingsMediaSessionId,
    QuickSettingsScene,
};
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct QuickSettingsLaidOutAudioSession {
    id: QuickSettingsAudioSessionId,
    bounds: DipRect,
    track: DipRect,
}

impl QuickSettingsLaidOutAudioSession {
    #[must_use]
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct QuickSettingsLaidOutAudioOutput {
    id: QuickSettingsAudioOutputId,
    bounds: DipRect,
}

impl QuickSettingsLaidOutAudioOutput {
    #[must_use]
...
pub struct QuickSettingsLayout {
    ...
    sound_details_bounds: Option<DipRect>,
    audio_sessions: Vec<QuickSettingsLaidOutAudioSession>,
    audio_outputs: Vec<QuickSettingsLaidOutAudioOutput>,
    audio_settings_bounds: Option<DipRect>,
    audio_spatial_bounds: Option<DipRect>,
    audio_empty_bounds: Option<DipRect>,
    content_height: f32,
}
...
impl QuickSettingsLayout {
    ...
    #[must_use]
    pub const fn sound_details_bounds(&self) -> Option<DipRect> {
        self.sound_details_bounds
    }
    #[must_use]
    pub fn audio_sessions(&self) -> &[QuickSettingsLaidOutAudioSession] {
        &self.audio_sessions
    }
    #[must_use]
    pub fn audio_outputs(&self) -> &[QuickSettingsLaidOutAudioOutput] {
        &self.audio_outputs
    }
    #[must_use]
    pub const fn audio_settings_bounds(&self) -> Option<DipRect> {
        self.audio_settings_bounds
    }
    #[must_use]
    pub const fn audio_spatial_bounds(&self) -> Option<DipRect> {
        self.audio_spatial_bounds
    }
    #[must_use]
    pub const fn audio_empty_bounds(&self) -> Option<DipRect> {
        self.audio_empty_bounds
    }
    #[must_use]
    ...
    #[must_use]
    pub fn audio_session_value_at(&self, id: QuickSettingsAudioSessionId, x: f32) -> Option<u8> {
        self.audio_sessions
            .iter()
            .find(|session| session.id == id)
            .map(|session| {
                let ratio = ((x - session.track.x) / session.track.width).clamp(0.0, 1.0);
                (ratio * 100.0).round() as u8
            })
    }
    ...
    pub fn hit_test(&self, point: DipPoint) -> Option<QuickSettingsHit> {
        ...
        if let Some(session) = self
            .audio_sessions
            .iter()
            .find(|session| contains(session.bounds, point))
        {
            let ratio = ((point.x - session.track.x) / session.track.width).clamp(0.0, 1.0);
            return Some(QuickSettingsHit::AudioSessionSlider {
                id: session.id,
        ...
        if let Some(output) = self
            .audio_outputs
            .iter()
            .find(|output| contains(output.bounds, point))
        {
            return Some(QuickSettingsHit::AudioOutput(output.id));
        }
        if let Some(choice) = self
            .choices
            .iter()
        ...
        if self
            ...
            .is_some_and(|bounds| contains(bounds, point))
        {
            return Some(QuickSettingsHit::SoundDetails);
        }
        if self
            .audio_settings_bounds
            .is_some_and(|bounds| contains(bounds, point))
        {
            return Some(QuickSettingsHit::AudioSettings);
        }
        if self
            .audio_spatial_bounds
            .is_some_and(|bounds| contains(bounds, point))
        {
            return Some(QuickSettingsHit::AudioSpatial);
        }
        if self
            .edit_bounds
            .is_some_and(|bounds| contains(bounds, point))
...
pub fn layout_quick_settings(scene: &QuickSettingsScene, surface: DipRect) -> QuickSettingsLayout {
    ...
    if let Some(media_choices) = scene.media_choices() {
        ...
        return QuickSettingsLayout {
            ...
            edit_bounds: None,
            sound_details_bounds: None,
            audio_sessions: Vec::new(),
            audio_outputs: Vec::new(),
            audio_settings_bounds: None,
            audio_spatial_bounds: None,
            audio_empty_bounds: None,
            content_height,
        };
    }
    ...
    if let Some(audio) = scene.audio_panel() {
        let back_bounds =
            (!audio.standalone()).then(|| DipRect::new(content.x, y + 6.0, 36.0, 36.0));
        y += SUBMENU_HEADER_HEIGHT + GAP;
        let master_bounds = DipRect::new(
            content.x,
            y + AUDIO_SECTION_LABEL_HEIGHT,
            content.width,
            SLIDER_HEIGHT,
        );
        let sliders = vec![slider_layout(audio.master().kind(), master_bounds)];
        y += AUDIO_SECTION_LABEL_HEIGHT + SLIDER_HEIGHT + GAP;

        let mut audio_sessions = Vec::new();
        y += AUDIO_SECTION_LABEL_HEIGHT;
        let audio_empty_bounds = if audio.sessions().is_empty() {
            let bounds = DipRect::new(content.x, y, content.width, 42.0);
            y += 42.0 + GAP;
            Some(bounds)
        } else {
            for session in audio.sessions().iter().take(3) {
                let bounds = DipRect::new(content.x, y, content.width, AUDIO_SESSION_HEIGHT);
                audio_sessions.push(QuickSettingsLaidOutAudioSession {
                    id: session.id(),
                    bounds,
                    track: DipRect::new(
                        bounds.x + 34.0,
        ...
        let audio_spatial_bounds = audio.spatial_audio().then(|| {
            ...
            y += AUDIO_OUTPUT_HEIGHT;
            bounds
        });
        y += SECTION_GAP;
        let audio_settings_bounds = DipRect::new(content.x, y, content.width, FOOTER_HEIGHT);
        y += FOOTER_HEIGHT;
        let content_height = y + OUTER + offset - surface.y;
        return QuickSettingsLayout {
            content_bounds: content,
            ...
            audio_sessions,
            audio_outputs,
            audio_settings_bounds: Some(audio_settings_bounds),
            audio_spatial_bounds,
            audio_empty_bounds,
            content_height,
        };
    ...
    let mut sound_details_bounds = None;
    ...
    if let Some(sound) = scene.sound() {
        ...
        sound_details_bounds = Some(DipRect::new(
            bounds.x,
            bounds.y,
            bounds.width,
            SECTION_PADDING + SECTION_HEADER_HEIGHT,
        ));
...
fn slider_layout(kind: QuickControlKind, bounds: DipRect) -> QuickSettingsLaidOutSlider {
    let track = if matches!(
        kind,
        QuickControlKind::Brightness | QuickControlKind::Volume
    ) {
        DipRect::new(bounds.x, bounds.y + 4.0, bounds.width, 28.0)
    } else {
        DipRect::new(
            bounds.x + 36.0,
