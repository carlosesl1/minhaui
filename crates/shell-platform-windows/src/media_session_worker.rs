        ...
        match signal {
            ...
            WorkerSignal::Command(MediaWorkerCommand::Refresh) | WorkerSignal::Refresh(None) => {
                ...
                if let Ok(session) = manager.GetCurrentSession()
                    && let Ok(current) = crate::win32_media_sessions::session_id(&session)
                {
...
fn publish_refreshed(
    ...
    let result =
        sync_subscriptions(manager, sender, refresh_pending, subscriptions).and_then(|()| {
            publish_state.generation = publish_state.generation.saturating_add(1);
            crate::win32_media_sessions::capture_snapshot(
                manager,
                publish_state.generation,
                activity,
            )
        });
    publish_snapshot(result, publish_state.wake_window);
}
